...
class TestWeFaxStations:
    ...
    def test_station_has_required_fields(self):
        ...
        required = {"name", "callsign", "country", "city", "coordinates", "frequencies", "ioc", "lpm", "schedule"}
...
class TestWeFaxRoutes:
    ...
    def test_start_success(self, client):
        ...
        with (
            patch("routes.wefax.get_wefax_decoder", return_value=mock_decoder),
    ...
    def test_start_device_busy(self, client):
        ...
        with (
            patch("routes.wefax.get_wefax_decoder", return_value=mock_decoder),
            patch("routes.wefax.app_module.claim_sdr_device", return_value="Device 0 in use by pager"),
        ):
            response = client.post(
                "/wefax/start",
