...
def validate_device_schema(device: dict, context: str = "") -> list[str]:
    ...
    errors = []
    required_fields = ["device_id", "address", "rssi_current", "last_seen", "seen_count"]
...
def validate_tracker_schema(tracker: dict, context: str = "") -> list[str]:
    ...
    required_fields = ["device_id", "address", "tracker"]
    for field in required_fields:
    ...
    def test_devices_endpoint(self):
        ...
        try:
            ...
            if data.get("devices"):
                ...
                self._check(
                    "Has tracker fields", "is_tracker" in device, "New tracker field missing (backwards compat issue)"
                )
