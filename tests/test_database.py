...
class TestDeviceCorrelations:
    ...
    def test_add_and_get_correlation(self, temp_db):
        ...
        add_correlation(
            wifi_mac="AA:AA:AA:AA:AA:AA", bt_mac="BB:BB:BB:BB:BB:BB", confidence=0.85, metadata={"reason": "timing"}
        )

        correlations = get_correlations(min_confidence=0.5)

        assert len(correlations) >= 1
        found = next((c for c in correlations if c["wifi_mac"] == "AA:AA:AA:AA:AA:AA"), None)
    ...
    def test_correlation_upsert(self, temp_db):
        ...
        matching = [c for c in correlations if c["wifi_mac"] == "AA:AA:AA:AA:AA:AA"]
