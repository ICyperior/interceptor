...
class TestOokRoutes:
    ...
    def test_start_validates_frequency(self, client):
        _login_session(client)
        resp = client.post("/ook/start", json={"frequency": "invalid"}, content_type="application/json")
        assert resp.status_code == 400

    def test_start_validates_encoding(self, client):
        _login_session(client)
        resp = client.post("/ook/start", json={"encoding": "invalid_enc"}, content_type="application/json")
        assert resp.status_code == 400

    def test_start_validates_timing_params(self, client):
        _login_session(client)
        resp = client.post("/ook/start", json={"short_pulse": "not_a_number"}, content_type="application/json")
        assert resp.status_code == 400

    def test_start_rejects_negative_frequency(self, client):
        _login_session(client)
        resp = client.post("/ook/start", json={"frequency": "-5"}, content_type="application/json")
        assert resp.status_code == 400

    def test_start_rejects_out_of_range_timing(self, client):
        """Timing params that exceed server-side max should be rejected."""
        _login_session(client)
        resp = client.post("/ook/start", json={"short_pulse": 999999}, content_type="application/json")
        assert resp.status_code == 400

    def test_start_rejects_negative_timing(self, client):
        _login_session(client)
        resp = client.post("/ook/start", json={"min_bits": -1}, content_type="application/json")
        assert resp.status_code == 400
    ...
    def test_start_success_mocked(self, client, monkeypatch):
        ...
        resp = client.post("/ook/start", json={"frequency": "433.920"}, content_type="application/json")
        data = resp.get_json()
        assert resp.status_code == 200
